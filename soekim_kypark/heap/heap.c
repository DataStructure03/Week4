...
int pushHeapData(Heap* heap, HeapData data)
{
	...
	for (int childIndex = heap->currentCount - 1,
		...
		parentIndex != ERROR &&
		heap->data[childIndex].priority > heap->data[parentIndex].priority;
...
// 		biggerChild = right;
		...
		biggerChild = right;
	if (heap->data[parentIndex].priority < heap->data[biggerChild].priority)
	{
		swap(&(heap->data[parentIndex]), &(heap->data[biggerChild]));
		heapify(heap, biggerChild);
	}
}
